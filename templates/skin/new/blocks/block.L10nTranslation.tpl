    ...
    <div class="cl">
        ...
        {if $bAllowTopicTranslation}
            <div class="{if ($oTopicOriginal || $aTopicTranslates)}bottom{else}cr{/if}">
                <a href="{router page='topic'}add/translate/{$oTopic->getId()}" alt="{$aLang.plugin.l10n.l10n_topic_translate}" title="{$aLang.l10n_topic_translate}">{$aLang.l10n_topic_translate}</a>
            </div>
        {/if}
