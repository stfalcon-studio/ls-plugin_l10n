    ...
    <div class="cl">
            ...
            {if $oTopicOriginal}
                ...
                <div class="block-lang">
                    <img src="{$sTemplateWebPathPluginL10n}images/flags/{$sTopicLang}.png"
                         title="{$aLang.$l10n_lang_this}" alt="{$aLang.$l10n_lang_this}"/>
        ...
        {if $bAllowTopicTranslation}
            <div class="{if ($oTopicOriginal || $aTopicTranslates)}bottom{else}cr{/if}">
                <a href="{router page='topic'}add/translate/{$oTopic->getId()}" alt="{$aLang.l10n_topic_translate}" title="{$aLang.l10n_topic_translate}">{$aLang.l10n_topic_translate}</a>
            </div>
        {/if}
